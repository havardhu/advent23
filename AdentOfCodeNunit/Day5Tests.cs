using AdventOfCodeLogic;

namespace AdentOfCodeNunit;
 [TestFixture]
public class Day5Tests
{
    [SetUp]
    public void Setup()
    {
    }

    [Test]
    public async Task TestPart1()
    {
        ITestDataProvider dataProvider = new AdventOfCodeLogic.StringTestDataProvider(@"seeds: 79 14 55 13

seed-to-soil map:
50 98 2
52 50 48

soil-to-fertilizer map:
0 15 37
37 52 2
39 0 15

fertilizer-to-water map:
49 53 8
0 11 42
42 0 7
57 7 4

water-to-light map:
88 18 7
18 25 70

light-to-temperature map:
45 77 23
81 45 19
68 64 13

temperature-to-humidity map:
0 69 1
1 0 69

humidity-to-location map:
60 56 37
56 93 4");

        var logic = new AdventOfCodeLogic.Day5();
        var result = await logic.SolvePart1(dataProvider);
        Assert.That(result, Is.EqualTo(35));
    }

        [Test]
    public async Task RunPart1()
    {
        ITestDataProvider dataProvider = new AdventOfCodeLogic.SessionDataProvider(5);

        var logic = new AdventOfCodeLogic.Day5();
        var result = await logic.SolvePart1(dataProvider);
        Assert.That(result, Is.EqualTo(107430936));
    }

    
    [Test]
    public async Task TestPart2()
    {
        ITestDataProvider dataProvider = new AdventOfCodeLogic.StringTestDataProvider(@"seeds: 79 14 55 13

seed-to-soil map:
50 98 2
52 50 48

soil-to-fertilizer map:
0 15 37
37 52 2
39 0 15

fertilizer-to-water map:
49 53 8
0 11 42
42 0 7
57 7 4

water-to-light map:
88 18 7
18 25 70

light-to-temperature map:
45 77 23
81 45 19
68 64 13

temperature-to-humidity map:
0 69 1
1 0 69

humidity-to-location map:
60 56 37
56 93 4");

        var logic = new AdventOfCodeLogic.Day5();
        var result = await logic.SolvePart2BruteForce(dataProvider);
        Assert.That(result, Is.EqualTo(46));
    }

      
    [Test]
    public async Task SolvePart2()
    {
        ITestDataProvider dataProvider = new AdventOfCodeLogic.SessionDataProvider(5);

        var logic = new AdventOfCodeLogic.Day5();
        var result = await logic.SolvePart2BruteForce(dataProvider);
        Assert.That(result, Is.EqualTo(46));
    }

    [Test]
    public async Task TestPart2Fast()
    {
        ITestDataProvider dataProvider = new AdventOfCodeLogic.StringTestDataProvider(@"seeds: 79 14 55 13

seed-to-soil map:
50 98 2
52 50 48

soil-to-fertilizer map:
0 15 37
37 52 2
39 0 15

fertilizer-to-water map:
49 53 8
0 11 42
42 0 7
57 7 4

water-to-light map:
88 18 7
18 25 70

light-to-temperature map:
45 77 23
81 45 19
68 64 13

temperature-to-humidity map:
0 69 1
1 0 69

humidity-to-location map:
60 56 37
56 93 4");

        var logic = new AdventOfCodeLogic.Day5Fast1337Solver();
        var result = await logic.SolvePart2Recursive(dataProvider);
        Assert.That(result, Is.EqualTo(46));
    }


    [Test]
    public async Task SolvePart2Fast()
    {
        ITestDataProvider dataProvider = new SessionDataProvider(5);
        var logic = new AdventOfCodeLogic.Day5Fast1337Solver();
        var result = await logic.SolvePart2Recursive(dataProvider);
        Assert.That(result, Is.EqualTo(23738616));
    }
}